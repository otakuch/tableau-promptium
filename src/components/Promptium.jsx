          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: isMobile ? '4px' : '6px', flexWrap: 'wrap', marginBottom: isMobile ? '12px' : '16px' }}>
            {exampleElements.map((el, index) => (
              <React.Fragment key={el.id}>
                <div style={{
                  width: isMobile ? '36px' : '44px', height: isMobile ? '36px' : '44px', background: el.color,
                  borderRadius: '6px',
                  display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                }}>
                  <span style={{ fontSize: isMobile ? '0.45rem' : '0.55rem', color: 'rgba(255,255,255,0.8)' }}>{el.id}</span>
                  <span style={{ fontSize: isMobile ? '0.8rem' : '1rem', fontWeight: '700', color: '#FFFFFF' }}>{el.symbol}</span>
                </div>
